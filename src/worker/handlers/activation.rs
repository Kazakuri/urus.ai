use askama::Template;
use faktory::Job;
use lettre::{smtp::ClientSecurity, SmtpClient, Transport};
...
pub fn activation(job: &Job) -> Result<()> {
    match job.args() {
        ...
        [user_body, token_body] => {
            let user_args: String =
                serde_json::from_value(user_body.to_owned()).expect("Unable to parse job");
            let token_args: String =
                serde_json::from_value(token_body.to_owned()).expect("Unable to parse job");

            match (
                serde_json::from_str::<User>(&user_args),
                serde_json::from_str::<UserToken>(&token_args),
            ) {
                ...
                (Ok(user), Ok(token)) => {
                    ...
                    let mut mailer = SmtpClient::new("127.0.0.1:1025", ClientSecurity::None)
                        .unwrap()
                        .transport();
