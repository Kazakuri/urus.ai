...
use urusai_lib::models::short_url::{NewShortURL, ShortURL};

lazy_static! {
    static ref BLACKLIST: Vec<&'static str> = vec![
        "urus.ai", "polr.me", "bit.ly", "is.gd", "tiny.cc", "adf.ly", "ur1.ca", "goo.gl", "ow.ly",
        "j.mp", "t.co",
    ];
}
...
pub fn create(conn: &Connection, msg: CreateURL) -> <CreateURL as Message>::Result {
    ...
    let optional_user_id = match msg.user_id {
        Some(optional_user_id) => {
            Some(Uuid::parse_str(&optional_user_id).expect("Invalid UUID supplied"))
        }
        None => None,
    };
    ...
    if !url_slug
        .chars()
        .all(|c| nanoid::alphabet::SAFE.iter().any(|x| *x == c))
    {
    ...
    let result = diesel::insert_into(urls).values(&new_url).execute(conn);
