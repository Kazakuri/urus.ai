...
pub fn read(conn: &Connection, msg: &ReadURL) -> <ReadURL as Message>::Result {
    ...
    let item = urls.filter(slug.eq(&msg.slug)).first::<ShortURL>(conn);
    ...
    match item {
        Ok(expanded_url) => {
            ...
            if visit_update.is_err() {
                warn!(
                    "Updating visit count failed for expanded url: {}",
                    expanded_url.slug
                );
            }
...
mod tests {
    ...
    fn create_url(conn: &crate::db::implementation::Connection, slug: Option<String>) -> ShortURL {
        let result = crate::db::implementation::url::create(
            &conn,
            CreateURL {
    ...
    fn visit_count(conn: &crate::db::implementation::Connection, short_url: &ShortURL) -> i64 {
        ...
        urls.filter(slug.eq(&short_url.slug))
            .first::<ShortURL>(conn)
    ...
    fn success() {
        ...
        conn.test_transaction::<_, Error, _>(|| {
            ...
            let read_url = read(&conn, &ReadURL { slug: url.slug }).expect("Failed to read URL");
